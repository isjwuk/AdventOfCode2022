...
class Monkey
{
    ...
    [int]$monkeyOnFalse) 
    {
       $this.monkeyNumber=[int]$monkeyNumber
       $this.items=$items
...
for($position=0; $position -le $inputFile.Length; $position+=7){
    ...
    $monkeyNumber=[int](($inputFile[$position] -split(" "))[1]).TrimEnd(":")
    [System.Collections.ArrayList]$items=@((($inputFile[$position+1] -split(":"))[1].trim() -split(",")).trim()| ForEach-Object {[int]$_})
    ...
    $monkeyOnFalse=[int]($inputFile[$position+5] -split(" to monkey "))[1]
        ...
        if($monkey.items) {
            ...
            Foreach($item in $monkey.items){
                ...
                if($monkey.operationValue -eq "old"){
                    ...
                    $value=[int]$monkey.operationValue
                }
                #Get worried
                ...
                if($item/$monkey.testDivisibleBy -eq [math]::Round($item/$monkey.testDivisibleBy)){
                    ...
                    $null=$monkeys[$monkey.monkeyOnTrue].items.add([int]$item)
                    ...
                    $null=$monkeys[$monkey.monkeyOnFalse].items.add([int]$item)
                }
            }
